...
- **偏离规范（用户明示要求）**：阅读器底栏的进度滑杆删除。`IOS_PORT_PROMPT.md` §192 要求底栏含「进度滑杆 = PreviewSession」，但用户按 Apple Books 的形态要求重做控件，且明确指出这条滑杆没有用。§183 要求的 `PreviewSession` 引擎仍完整移植并由引擎测试覆盖，`ReaderModel` 的 `beginScrub/scrub/endScrub` 也保留——删掉的是入口，不是机制，目前该机制没有界面入口。若日后要恢复，接回这三个方法即可。
