...
- S5c 书架（对齐上游 Room v10 与书架改版）：根部改为三个固定分段 书架/继续阅读/稍后再读（`LibraryTab`），各自记住布局与排序（`library_tab_presentations_v1`，默认 智能/最近阅读↓/加入时间↓）；排序新增"智能"（有更新的书优先：最近读过的在前，其次来源 `lastUpdatedDate`，再次检测时间，其余按手动顺序；显式排序从不分区）；"仅有更新"筛选只在书架分段根部生效并持久化（`library_show_updates_only`）。系统入口 `未读更新`/`最近阅读` 退休（前者由收件箱取代、后者是排序），`来源休眠` 保留为快捷栏入口，收藏夹快捷栏是 iOS 自有的设计继续保留。
- 网站镜像作为书架快捷栏入口（`mirror:<sourceId>`），从书架栈打开 `RemoteLibraryScreen`（`LibraryRoute.mirror`），封面走书架的封面提供者、不打开来源流；只在镜像里的书有更新时，以 `localMembership = false` 的合成条目出现在"有更新"投影中，打开它不产生钉住。
- 本地书架搜索（`LibrarySearch`/`LibrarySearchModel`）：NFKC→小写→裁剪→合并空白后子串匹配，收藏夹（手动/智能）在前、书在后，均按规范化标题排序；语料 = 钉住条目 ∪ 未钉住的稍后再读；空输入给推荐（最近读过→最近加入，取 6；收藏夹按展示顺序取 6）；120 ms 去抖、提交立即生效、上限 100 字。零联网。
- 详情页主按钮语义：未在书架为"加入书架"一键加入；已在书架时主按钮弹出本地移除确认（评分、标签、稍后再读、进度保留，网站收藏不受影响）；稍后再读在没有本地记录时先创建记录。
- 界面偏好重置（更多 → 数据）：只清外观、书架展示（快捷栏顺序/锁定/隐藏入口/网站分组/分段展示/仅有更新）与阅读器排版键；`source_flow_*` 恢复快照、`last_applied_import_digest`、`official_repository_seeded` 保留。
- 书架封面改为"缓存优先、缺失时经来源通道按需下载"（`LazySourceCoverFetcher`），取代 M4 的"书架只读缓存、绝不打开通道"：从网站收藏复制或导入的书从未被浏览过，其封面在旧规则下永远是占位；按需打开的是与浏览共用的同一条通道，随应用退后台关闭。封面失败占位里显示失败原因码（宿主常量），便于读者反馈。
- 封面响应的格式由**字节**而非 `Content-Type` 决定（JPEG/PNG/GIF/WebP 魔数）：真实站点把 WebP 标成 `image/jpeg`、或用 `application/octet-stream`，此前一律被拒为 `DECODE`；标头与字节矛盾（例如 HTML 页标成 JPEG）仍拒绝，并把声明的类型折成受限令牌记入诊断（`type-image-jpeg`）。
- 更多 tab 精简（用户 2026-09-17）：去掉"数据"说明页与"帮助"页（`HelpTopic` 删除），只留数据迁移与关于；"重置界面偏好"移到"显示"页底部；"显示"页的外观选择改为分区标题加三行选项，不再有一行不可点的"外观"标签；更新检查"按来源"区不再带说明文字；数据迁移导入说明只保留支持的格式。阅读方式的"双页"（两栏对开）在手机上没有可感知的差别，从两处选择器里去掉（引擎与存储枚举保留，已存的"双页"读为"翻页"）。
- 网站收藏页的多选改为显式模式（"选择"/长按进入，"完成"退出），未选择时行首不画圆圈。
- 阅读器进度显示仍按页计（`x/y 页`）而非上游的码点加权百分比：位置本身已是语义定位（`ReaderLocator`），跨字号恢复不依赖百分比；分页模式下页码对读者更诚实。记为不移植项。
- S6 界面（功能与上游一致，形态按 iOS）：浏览页改为"已安装 / 可安装"分段：已安装行是一个"进入"主按钮加下拉（搜索此来源 / 网站书库 / 登录 / 来源详情 / 卸载来源），空态直接给出"导入内容源包"；可安装页把所有启用仓库的缓存目录合并（`CatalogModel`），本地过滤名称/摘要/语言/标识/仓库名，每行一个按钮，按钮文字即状态（安装 / 更新 / 已安装 / 不兼容 / 已撤销 / 仓库暂不可用 / 正在准备），过期但曾验证过的目录以"可能已过期"提示继续展示但不可安装；进入可安装分段时若目录还没读过就刷新一次，之外只有下拉刷新联网。
- 扩展市场页改为"仓库管理"：官方仓库行只能停用不能移除；第三方行有"启用此仓库"开关与"移除仓库"；订阅链接先"检查链接"，目录答复后在同一页展开"确认仓库身份"（标识、目录地址、根密钥指纹、序号、包数、发布者指纹）再"确认订阅"。未知发布者的本地包在浏览页整页显示"需要发布者公钥"卡片。安装审批页把发布者指纹、新增能力与上限提高折进"授权差异（新增 N 项 · 上限提高 M 项）"。
- 详情页身份模块：封面 120×180，右侧依次为可展开标题（默认两行）、作者链接（点击作者搜索）、状态与"上次更新"、五星评分（只对书架书生效，点当前星清除）、分体按钮（主半边"加入书架 / 已在书架"，尾半边下拉打开"更多加入选项"）。分体按钮按内容宽度，不拉满。
- 书架更新条：运行中显示进度与取消；结束后显示摘要、"查看报告"与（非完成态的）"重试"，可关闭；干净完成只显示 2 秒（以持久的 finishedAt 计）。更新报告为独立页，逐条给出状态标签与原因标签，"更多报告 (n/total)"分页；原因令牌只在是宿主自造的 `source-…` 诊断时原样显示。
- `UpdateScheduler.register` 用 `nonisolated(unsafe)` 把系统交来的非 Sendable `BGProcessingTask` 递给主线程对象：处理闭包在此之外不再使用该对象，BGTask 自身方法线程安全。这是本仓库唯一的并发逃逸，记录在此。
- 用户把关后的书架调整（2026-09-17）：书架只保留网格与列表两种布局（纯文字的"紧凑"去掉）；书架搜索不再是独立页，点右上放大镜时搜索框以 `.snappy(0.3)` 从标题下滑出（`move(edge: .top) + opacity`），命中在同一网格内重排，收藏夹命中以胶片列在网格上方，取消即复原；快捷栏空态只放"新建收藏夹"胶片，"整理/收折"与收藏夹胶片同一样式且只在有内容时出现；长按封面弹出书的菜单，拖拽只属于"排序整理"；无休眠书时不显示"来源休眠"入口；来源首页加系统搜索栏，回车交给搜索页执行；首页筛选按钮改为矮圆角矩形。
- 更正（2026-09-17）：上文"分类器补丁已进上游"记错了——`Chachaanteng/tsuyomi-extensions@1d7062e` 与其后的 `53b7dc5`（0.2.33）都仍是 `sessionRemediation` 前置门。wenku8 现在把 Cloudflare 的被动评分脚本（`/cdn-cgi/challenge-platform/scripts/jsd/main.js`）注入到完整送达的详情页里，路径里的 `challenge-platform` 命中扩展的验证页判据，于是每本书详情都报"需要验证"，而浏览器打开正常。这是扩展的误判，宿主按规范不得改判；修法是在匹配验证页判据前先把这条被动脚本引用去掉（真验证页、登录页与扩展自带的 `challenge`/`login` 用例判定不变），已在 `Tsuyomi-main/tsuyomi-extensions` 打上同样的补丁由 CI 跑扩展测试套件验证，并在独立克隆的 `fix/wenku8-passive-cloudflare-script` 分支上备好上游提交。补丁记录在 `docs/FIXTURE_EXTENSION_PATCH.md`。
- 书架刷新报"需要验证"（2026-09-17）同样是扩展侧：wenku8 的 Cloudflare 规则对不带 Referer 的 `/modules/article/reader.php` 返回真的拦截页（403），带任意 Referer 即放行；目录请求带了书页 referrer，`update-check-v2` 请求没带。宿主对签名操作要求 `request.referrerUrl == policy.origin + referrerPath`（无声明则必须为空），这条约束不放宽；修法是扩展清单的 `updateCheck` 声明 `referrerPath: "/index.php"` 且请求发同一地址。fixture 扩展已打同样补丁由 CI 验证，记录在 `docs/FIXTURE_EXTENSION_PATCH.md`。
- 书架封面的第二次机会（2026-09-17）：`LibraryCoverProvider` 原先每个来源只建一次 provider、整个运行期不换，`SourceCoverProvider` 的失败状态也永久保留——于是来源更新（包摘要变了）、登录或登出（凭据分区变了）、或者一次因浏览通道关闭而取消的取图之后，书架封面要到下次启动才会再试；provider 构建失败一次更是整个运行期都没有封面。现在书架每次回到前台（`onAppear`）调用 `revalidate()`：绑定的包摘要或凭据状态已变的 provider 丢弃重建，仍有效的 provider 清掉失败项让下一次绘制重取；构建失败不再记入"已解析"。成功的封面不会因此重取。网站收藏页"打开不联网、只有刷新列表才读网站"的约定不变：未加入书架、也没打开过详情的收藏书，其封面地址要靠"刷新列表"重新拉取来更新。
- 详情页学习书架记录：书架行的封面地址来自写入它的那次拉取（网站收藏的 pull，或加入书架时的详情），扩展换了图床后旧行仍指向已失效的 `pic.wenku8.com`，而 0.2.33 又把该 origin 从授权名单里去掉，封面于是一直不显示。现在 `BookModel.load` 在线载入详情后，若 `books` 表已有此书的行，就把标题、作者、封面、规范地址、状态与标签更新为详情页所说（保留加入时间、更新键与未读标记），无变化不写；离线缓存页不学习，因为它可能正是写下旧图床的那一页。网站收藏页本身的行在"刷新列表"重新拉取时由 `mergeBook` 更新封面。
