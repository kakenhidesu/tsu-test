...
- 「目录」那一行**就是**进度条：平时显示「目录 · N%」与列表图标，手指沿它拖动时它变成轨道（填充 + 细竖条拇指），面板上方浮出章节名与页码；未移动的点按仍然打开目录。此前做成了两个控件（面板里一行 + 面板下面一条滑杆），既不是参考里的形态，那条独立滑杆还压在翻页视图的手势上，拖不动。
- 阅读器的设置面板与设置 tab 不再共用同一套控件呈现：面板按参考做成「标题 + ✕、一行常用控件（字号 ± / 阅读方式 / 翻页效果 / 外观）、其余进入『选项』」，设置 tab 仍是标准 Form。共用的是 `AppPreferences.reader` 这一份值，不是控件——同一份设置在弹出面板与设置列表里本来就该长得不一样，硬共用会让两边都别扭。「选项」里嵌的仍然是 `ReaderSettingsForm`，所以数值类设置只有一处定义。
- 阅读主题改为 Apple Books 的模型：六个主题（原始/安静/纸张/粗体/平静/专注）各带浅色与深色两套色板，由生效的外观（浅色/深色/匹配设备）决定画哪一套；安卓的五个固定色板（纸色/暖灰/夜墨/纯黑/护眼绿）删除。它们把"主题"和"明暗"绑成一件事，切深色就得换主题，而这两件事在 iOS 上本来是分开的。低刺激与纸色两类主题仍在（专注、纸张）。
- `ReaderTheme.rawValue` 不再是 transfer 文件里的词：`tsuyomi-transfer-v1.schema.json` 把 `theme` 固定为五个词（安卓也 `require` 它们），六个主题装不进去。导出走 `transferName`（原始/纸张/粗体/平静→`paper`、安静→`warmGray`、专注→`inkGreen`），导入走 `init(transferName:)`（`nightInk`→安静、`black`→原始），认不出的词一律忽略，不抛错。本地存储用新 rawValue；旧值恰好就是 transfer 词汇，因此读取时用同一条映射兜底，不另写只跑一次的迁移。外观不在 transfer 词汇里，导出不按外观推导。
- 粗体不换字面：验收表规定"仅换主题不改变 layout key"，更重的字面字宽不同，会挪动每一页的边界。粗体是"更重的墨"——在同一字形上做 fill-and-stroke（`strokeWidth` 负值，按字号百分比），字宽不变、不重排；配最大反差的黑白。
- 阅读器不再覆写子树的 `\.colorScheme`：主题色板由生效外观选出，页面的明暗与环境一致，覆写没有差异可传。此前需要覆写，是因为旧主题各自固定明暗，可能与系统相反。
- 「主题与设置」不再是系统 sheet，而是阅读器自己画在页面上的卡片：单一 detent 的 sheet 仍能往上拉再弹回，UIKit 没有关掉这个回弹的接口，而被设置的正是它身下那一页。卡片高度由内容决定，不再猜一个 400；点页面或向下拖关闭。「选项」仍是 sheet——它是一张 Form，medium/large 是对的。文件与类型名保持 `ReaderSettingsSheet`，与 `IOS_PORT_PROMPT.md` 的文件对照表一致。
- 字号改为档位（14/16/18/20/22/24/28/32）：面板上「字|字」两半各走一档，胶囊下方一排点标出当前档，默认 18 落在第三个点。`fontSize` 仍存 Double 并按 `fontSizeRange` 钳制，导入的 `fontScale` 可落到区间内任意值，显示与步进都取最近一档；设置 tab 的 Stepper 走同一组档位。安卓的连续滑杆不移植。
- 面板一行四个控件统一为 56×44 胶囊（此前外观是 44 圆钮，与另外两个胶囊不同形）。
- 「更多」面板：「主题与设置」行的"大小"文字删掉，尺寸这层意思由行尾的 AA 图标承担；「目录」行拖动时不再整行换成一条轨道，而是文字与图标留在原处、填充画在其下、百分比随手指更新；填充与竖线共用同一个长度 `width × fraction`，手指位置换算的 `x / width` 也是同一个数，三者不可能不一致。
- 书籍页「展开/收起」改 `.borderless`：List 行里默认样式的 Button 会把整行变成它的命中区域，行里的标签不是控件，点标签却等于点它。
- 设置 tab「显示」里的选择器由「主题」改名「外观」：「主题」现在指阅读主题。
- 书架封面与来源封面走的是同一个坑：`LibraryCoverProvider` 持有一批 `SourceCoverProvider`，只在**创建**它们时 `revision += 1`，而封面从磁盘读完是那些嵌套 observable 自己发布的变更，没人转发 → 冷启动书架空白，进出一次强制重绘才出图。两处现在都把子对象的 `objectWillChange` 转发给宿主。嵌套 ObservableObject 不会自动向上冒泡，这是本仓库第二次踩它。
- 「目录」行在**静止时**就画出已读比例与位置标记（拖动时标记加深）：这一行是读者查看"读到哪儿"的地方，一条只有拖动时才出现的进度条回答不了这个问题。
- 「目录」与「主题与设置」是两张分开的卡片而不是一张卡加分隔线：参考里就是分开的，而它们本来也是两件事——一件是章内位置，一件是页面外观。
