...
final class RepositoryIndexTests: XCTestCase {
    ...
    func testAWellFormedCatalogIsAccepted() throws {
        ...
        XCTAssertEqual(RepositoryIndexCodec.sequence(ofCached: try envelope(try signedObject(sequence: 7))), 7)
    }
    ...
    func testAnInsecureAddressOrKeyIsRefusedBeforeAnyRequest() throws {
        ...
        XCTAssertThrowsError(try ExtensionRepositoryClient.normalize(indexUrl: "https://example.org/index-v1.json?token=1"))
        XCTAssertThrowsError(try ExtensionRepositoryClient.normalize(indexUrl: "https://example.org"))
